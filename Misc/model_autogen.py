# coding: utf-8
from sqlalchemy import ARRAY, BigInteger, Boolean, CheckConstraint, Column, Date, DateTime, Float, Integer, Numeric, String, Table, Text, Time
...
class AllStravaActivity(db.Model):
    ...
    geom = db.Column(db.Geometry('MULTILINESTRING', 4326, from_text='ST_GeomFromEWKT', name='geometry'), index=True)
...
class CACounty(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, server_default=db.FetchedValue())
    geom = db.Column(db.Geometry('MULTIPOLYGON', 4326, from_text='ST_GeomFromEWKT', name='geometry'), index=True)
...
class Gpsdatum(db.Model):
    ...
    geom = db.Column(db.Geometry('POINT', 4326, from_text='ST_GeomFromEWKT', name='geometry'))
