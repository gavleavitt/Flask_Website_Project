...
function createSearchControl(layerGroup) {
  ...
  searchControl.on('search:locationfound', function(e) {
    ...
    if(e.layer._popup)
      ...
      searchedActivityOnly = actFilter(null, null, null, e.layer.feature.properties.name);
...
function actFilter(actType = null, userStartDate = null, userEndDate = null, actName) {
  ...
  filteredAct = L.geoJson(rawGeoJSON, {
      ...
      onEachFeature: onEachFeatureAct,
        ...
        if (actType == "Walk") {
            if ((feature.properties.type == "Walk") && (feature.properties.startDate.slice(0,10) >= userStartDate.slice(0,10)) && (feature.properties.startDate.slice(0,10) <= userEndDate.slice(0,10))) {
              populateDict(feature.properties,actDataDict);
              return true
            }
        } else if (actType == "Run") {
          ...
          filteredGroup.clearLayers();
          if (feature.properties.name == actName) {
            return true
          }
        }
			}
  ...
  // Document, populates dashboard info
  // populateDashboard(filteredGroup);
};

//var actDataDict = {"count":null, "dist":null, "elev":null, "calories":null, "time":null, "speed":null}
function populateDict(propertyData,actDataDict,clear) {
  if (clear == "True"){
    var actDataDict = {"count":null, "dist":null, "elev":null, "calories":null, "time":null, "speed":null}
  }

  actDataDict["count"] += 1
  actDataDict["dist"] += propertyData.distance;
  actDataDict["elev"] += propertyData.total_elevation_gain;
  actDataDict["calories"] += propertyData.calories;
  actDataDict["time"] += propertyData.elapsed_time;
  actDataDict["speed"] += propertyData.average_speed;
};

function updateDataPanels(actDataDict){
  document.getElementById('actCount').innerHTML = actDataDict["count"];
  document.getElementById('totalDist').innerHTML = actDataDict["dist"];
  document.getElementById('totalElev').innerHTML = actDataDict["elev"];
  document.getElementById('totalCalories').innerHTML = actDataDict["calories"];
  document.getElementById('totalTime').innerHTML = actDataDict["time"];
  document.getElementById('avgSpeed').innerHTML = (actDataDict["speed"]/actDataDict["count"]).toFixed(1);
};

// function populateDashboard(propertyData){
