...
from flask_application.WebAppProjects.LocationLiveTracker.modelsTracker import gpsPointModel, gpstracks, AOI, CaliforniaPlaces, CACounty
...
def getTrackerFeatCollection(datatype, reclimit):
    ...
    if datatype == "gpspoints":
        ...
        # flask_application.logger.debug(f"Queried time is: {recDateTime[0]}")
        utcTime = recDateTime[0].replace(tzinfo=pytz.utc)
        # flask_application.logger.debug(f"UTC time is: {utcTime }")
        # Convert from utc time to localtime
        localTime = utcTime.astimezone(pytz.timezone(recTZ[0]))
        # flask_application.logger.debug(f"Local time time is: {localTime}")
        ...
        # flask_application.logger.debug(f"Start of day in local is: {startofDayLocal}")
        startofDayUTC = startofDayLocal.astimezone(pytz.utc)
        # flask_application.logger.debug(f"Start of day in UTC is: {startofDayUTC}")
