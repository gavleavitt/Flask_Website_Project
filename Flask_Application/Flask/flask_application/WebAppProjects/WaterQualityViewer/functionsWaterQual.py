...
from flask_application.WebAppProjects.WaterQualityViewer import GoogleDriveUploadWaterQuality, DBQueriesWaterQuality, AWSS3Upload
import os
...
def genReSampleDict(tab, pdfDate):
    ...
    for row in range(1, len(tab)):
        ...
        if "sample" in tab[row][0]:
            ...
            # flask_application.logger.debug(f"Adding the following beach to the combined beaches list {tab[row][0]}")
